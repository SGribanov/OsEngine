...
### Step 4.2 - Nullable Annotations (Incremental Adoption #994-#996)

- **Status:** In Progress (increment block completed)
- **Plan item:** `refactoring_stage2_plan.md` -> Phase 4 / Step 4.2
- **Changes (first child serializer round-trip contracts):**
  - Updated tests in `project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs`:
  ...
  - Outcome:
    - locked direct serializer/parser round-trip behavior for three isolated child grid components.
- **Verification (outside sandbox, per dotnet-build-policy):**
  - `dotnet restore project/OsEngine/OsEngine.csproj --nologo` -> success
  - `dotnet restore project/OsEngine.Tests/OsEngine.Tests.csproj --nologo` -> success
  - `dotnet build project/OsEngine/OsEngine.csproj --no-restore --configuration Release --nologo -p:NoWarn=NU1900` -> success, 0 warnings, 0 errors
...
### Step 4.2 - Nullable Annotations (Incremental Adoption #997-#999)

- **Status:** In Progress (increment block completed)
- **Plan item:** `refactoring_stage2_plan.md` -> Phase 4 / Step 4.2
- **Changes (next child serializer round-trip contracts):**
  - Updated tests in `project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs`:
  ...
  - Outcome:
    - locked direct serializer/parser round-trip behavior for the remaining isolated child components outside `TrailingUp`.
- **Verification (outside sandbox, per dotnet-build-policy):**
  - `dotnet restore project/OsEngine/OsEngine.csproj --nologo` -> success
  - `dotnet restore project/OsEngine.Tests/OsEngine.Tests.csproj --nologo` -> success
  - `dotnet build project/OsEngine/OsEngine.csproj --no-restore --configuration Release --nologo -p:NoWarn=NU1900` -> success, 0 warnings, 0 errors
...
### Step 4.2 - Nullable Annotations (Incremental Adoption #1000)

- **Status:** In Progress (increment block completed)
- **Plan item:** `refactoring_stage2_plan.md` -> Phase 4 / Step 4.2
- **Changes (final isolated child serializer round-trip):**
  - Updated tests in `project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs`:
    - added `Stage2Step2_2_TrailingUp_GetSaveString_LoadFromString_ShouldRoundTrip`.
  - Outcome:
    - closed the last isolated child serializer/parser round-trip gap in `TrailingUp`.
- **Verification (outside sandbox, per dotnet-build-policy):**
  - `dotnet restore project/OsEngine/OsEngine.csproj --nologo` -> success
  - `dotnet restore project/OsEngine.Tests/OsEngine.Tests.csproj --nologo` -> success
  - `dotnet build project/OsEngine/OsEngine.csproj --no-restore --configuration Release --nologo -p:NoWarn=NU1900` -> success, 0 warnings, 0 errors
...
### Step 4.2 - Nullable Annotations (Incremental Adoption #1001)

- **Status:** In Progress (increment block completed)
- **Plan item:** `refactoring_stage2_plan.md` -> Phase 4 / Step 4.2
- **Changes (first full top-level composite persistence round-trip):**
  - Updated tests in `project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs`:
    - added `Stage2Step2_2_TradeGrid_GetSaveString_LoadFromString_ShouldRoundTrip`.
  - Outcome:
    - locked full `TradeGrid.GetSaveString() -> LoadFromString()` round-trip behavior across the prime section and all currently persisted child components.
- **Verification (outside sandbox, per dotnet-build-policy):**
  - `dotnet restore project/OsEngine/OsEngine.csproj --nologo` -> success
  - `dotnet restore project/OsEngine.Tests/OsEngine.Tests.csproj --nologo` -> success
  - `dotnet build project/OsEngine/OsEngine.csproj --no-restore --configuration Release --nologo -p:NoWarn=NU1900` -> success, 0 warnings, 0 errors
...
### Step 4.2 - Nullable Annotations (Incremental Adoption #1002)

- **Status:** In Progress (increment block completed)
- **Plan item:** `refactoring_stage2_plan.md` -> Phase 4 / Step 4.2
- **Changes (sparse top-level serialization shape):**
  - Updated tests in `project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs`:
    - added `Stage2Step2_2_TradeGrid_GetSaveString_WithNullChildSections_ShouldKeepEmptySectionShape`.
  - Outcome:
    - locked `TradeGrid.GetSaveString()` shape on sparse state where all persisted child references are null.
    - verified the prime section still serializes top-level field defaults while child sections remain empty but structurally present.
- **Verification (outside sandbox, per dotnet-build-policy):**
  - `dotnet restore project/OsEngine/OsEngine.csproj --nologo` -> success
  - `dotnet restore project/OsEngine.Tests/OsEngine.Tests.csproj --nologo` -> success
  - `dotnet build project/OsEngine/OsEngine.csproj --no-restore --configuration Release --nologo -p:NoWarn=NU1900` -> success, 0 warnings, 0 errors
...
### Step 4.2 - Nullable Annotations (Incremental Adoption #1003)

- **Status:** In Progress (increment block completed)
- **Plan item:** `refactoring_stage2_plan.md` -> Phase 4 / Step 4.2
- **Changes (sparse top-level load behavior with empty child sections):**
  - Updated tests in `project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs`:
    - added `Stage2Step2_2_TradeGrid_LoadFromString_WithEmptyChildSections_ShouldKeepExistingChildValues`.
  - Outcome:
    - locked the sparse-load counterpart to the sparse-save contract.
    - verified top-level prime fields parse while already-initialized child components preserve their current values when the payload sections are empty.
- **Verification (outside sandbox, per dotnet-build-policy):**
  - `dotnet restore project/OsEngine/OsEngine.csproj --nologo` -> success
  - `dotnet restore project/OsEngine.Tests/OsEngine.Tests.csproj --nologo` -> success
  - `dotnet build project/OsEngine/OsEngine.csproj --no-restore --configuration Release --nologo -p:NoWarn=NU1900` -> success, 0 warnings, 0 errors
...
### Step 4.2 - Nullable Annotations (Incremental Adoption #1004)

- **Status:** In Progress (increment block completed)
- **Plan item:** `refactoring_stage2_plan.md` -> Phase 4 / Step 4.2
- **Changes (top-level load with null child references and present child payloads):**
  - Updated tests in `project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs`:
    - added `Stage2Step2_2_TradeGrid_LoadFromString_WithNullChildReferencesAndPresentPayloads_ShouldApplyPrimeAndNotThrow`.
  - Outcome:
    - locked null-safe behavior when child payload sections are present but the corresponding top-level child references are absent.
    - verified prime fields still parse and the method remains safe.
- **Verification (outside sandbox, per dotnet-build-policy):**
  - `dotnet restore project/OsEngine/OsEngine.csproj --nologo` -> success
  - `dotnet restore project/OsEngine.Tests/OsEngine.Tests.csproj --nologo` -> success
  - `dotnet build project/OsEngine/OsEngine.csproj --no-restore --configuration Release --nologo -p:NoWarn=NU1900` -> success, 0 warnings, 0 errors
...
### Step 4.2 - Nullable Annotations (Incremental Adoption #1005)

- **Status:** In Progress (increment block completed)
- **Plan item:** `refactoring_stage2_plan.md` -> Phase 4 / Step 4.2
- **Changes (top-level prime-only partial load):**
  - Updated tests in `project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs`:
    - added `Stage2Step2_2_TradeGrid_LoadFromString_WithPrimeOnlyPayload_ShouldApplyPrimeAndKeepExistingChildValues`.
  - Outcome:
    - locked partial-load behavior when only the prime section is present and child sections are omitted entirely.
    - verified prime fields parse while existing child components preserve their previous values.
- **Verification (outside sandbox, per dotnet-build-policy):**
  - `dotnet restore project/OsEngine/OsEngine.csproj --nologo` -> success
  - `dotnet restore project/OsEngine.Tests/OsEngine.Tests.csproj --nologo` -> success
  - `dotnet build project/OsEngine/OsEngine.csproj --no-restore --configuration Release --nologo -p:NoWarn=NU1900` -> success, 0 warnings, 0 errors
...
### Step 4.2 - Nullable Annotations (Incremental Adoption #1006)

- **Status:** In Progress (increment block completed)
- **Plan item:** `refactoring_stage2_plan.md` -> Phase 4 / Step 4.2
- **Changes (top-level selective child-section partial load):**
  - Updated tests in `project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs`:
    - added `Stage2Step2_2_TradeGrid_LoadFromString_WithOnlyStopBySection_ShouldApplyPrimeAndTargetChildOnly`.
  - Outcome:
    - locked selective child-section behavior where only the `StopBy` section is present.
    - verified the target child updates while unrelated child components preserve their previous values.
- **Verification (outside sandbox, per dotnet-build-policy):**
  - `dotnet restore project/OsEngine/OsEngine.csproj --nologo` -> success
  - `dotnet restore project/OsEngine.Tests/OsEngine.Tests.csproj --nologo` -> success
  - `dotnet build project/OsEngine/OsEngine.csproj --no-restore --configuration Release --nologo -p:NoWarn=NU1900` -> success, 0 warnings, 0 errors
...
### Step 4.2 - Nullable Annotations (Incremental Adoption #1007)

- **Status:** In Progress (increment block completed)
- **Plan item:** `refactoring_stage2_plan.md` -> Phase 4 / Step 4.2
- **Changes (top-level selective `GridCreator` child-section partial load):**
  - Updated tests in `project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs`:
    - added `Stage2Step2_2_TradeGrid_LoadFromString_WithOnlyGridCreatorSection_ShouldApplyPrimeAndTargetChildOnly`.
  - Outcome:
    - locked selective child-section behavior where only the `GridCreator` section is present.
    - verified the target child updates while unrelated child components preserve their previous values.
- **Verification (outside sandbox, per dotnet-build-policy):**
  - `dotnet restore project/OsEngine/OsEngine.csproj --nologo` -> success
  - `dotnet restore project/OsEngine.Tests/OsEngine.Tests.csproj --nologo` -> success
...
### Step 4.2 - Nullable Annotations (Incremental Adoption #1008)

- **Status:** In Progress (increment block completed)
- **Plan item:** `refactoring_stage2_plan.md` -> Phase 4 / Step 4.2
- **Changes (top-level selective `StopAndProfit` child-section partial load):**
  - Updated tests in `project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs`:
    - added `Stage2Step2_2_TradeGrid_LoadFromString_WithOnlyStopAndProfitSection_ShouldApplyPrimeAndTargetChildOnly`.
  - Outcome:
    - locked selective child-section behavior where only the `StopAndProfit` section is present.
    - verified the target child updates while unrelated child components preserve their previous values.
- **Verification (outside sandbox, per dotnet-build-policy):**
  - `dotnet restore project/OsEngine/OsEngine.csproj --nologo` -> success
  - `dotnet restore project/OsEngine.Tests/OsEngine.Tests.csproj --nologo` -> success
...
### Step 4.2 - Nullable Annotations (Incremental Adoption #1009)

- **Status:** In Progress (increment block completed)
- **Plan item:** `refactoring_stage2_plan.md` -> Phase 4 / Step 4.2
- **Changes (top-level selective `AutoStarter` child-section partial load):**
  - Updated tests in `project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs`:
    - added `Stage2Step2_2_TradeGrid_LoadFromString_WithOnlyAutoStarterSection_ShouldApplyPrimeAndTargetChildOnly`.
  - Outcome:
    - locked selective child-section behavior where only the `AutoStarter` section is present.
    - verified the target child updates while unrelated child components preserve their previous values.
- **Verification (outside sandbox, per dotnet-build-policy):**
  - `dotnet restore project/OsEngine/OsEngine.csproj --nologo` -> success
  - `dotnet restore project/OsEngine.Tests/OsEngine.Tests.csproj --nologo` -> success
...
### Step 4.2 - Nullable Annotations (Incremental Adoption #1010)

- **Status:** In Progress (increment block completed)
- **Plan item:** `refactoring_stage2_plan.md` -> Phase 4 / Step 4.2
- **Changes (top-level selective `ErrorsReaction` child-section partial load):**
  - Updated tests in `project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs`:
    - added `Stage2Step2_2_TradeGrid_LoadFromString_WithOnlyErrorsReactionSection_ShouldApplyPrimeAndTargetChildOnly`.
  - Outcome:
    - locked selective child-section behavior where only the `ErrorsReaction` section is present.
    - verified the target child updates while unrelated child components preserve their previous values.
- **Verification (outside sandbox, per dotnet-build-policy):**
  - `dotnet restore project/OsEngine/OsEngine.csproj --nologo` -> success
  - `dotnet restore project/OsEngine.Tests/OsEngine.Tests.csproj --nologo` -> success
...
### Step 4.2 - Nullable Annotations (Incremental Adoption #1011)

- **Status:** In Progress (increment block completed)
- **Plan item:** `refactoring_stage2_plan.md` -> Phase 4 / Step 4.2
- **Changes (top-level selective `TrailingUp` child-section partial load):**
  - Updated tests in `project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs`:
    - added `Stage2Step2_2_TradeGrid_LoadFromString_WithOnlyTrailingUpSection_ShouldApplyPrimeAndTargetChildOnly`.
  - Outcome:
    - locked selective child-section behavior where only the `TrailingUp` section is present.
    - verified the target child updates while unrelated child components preserve their previous values.
- **Verification (outside sandbox, per dotnet-build-policy):**
  - `dotnet restore project/OsEngine/OsEngine.csproj --nologo` -> success
  - `dotnet restore project/OsEngine.Tests/OsEngine.Tests.csproj --nologo` -> success
...
### Step 4.2 - Nullable Annotations (Incremental Adoption #1012)

- **Status:** In Progress (increment block completed)
- **Plan item:** `refactoring_stage2_plan.md` -> Phase 4 / Step 4.2
- **Changes (top-level selective `NonTradePeriods` child-section partial load):**
  - Updated tests in `project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs`:
    - added `Stage2Step2_2_TradeGrid_LoadFromString_WithOnlyNonTradePeriodsSection_ShouldApplyPrimeAndTargetChildOnly`.
  - Outcome:
    - locked selective child-section behavior where only the `NonTradePeriods` section is present.
    - verified the target child updates while unrelated child components preserve their previous values.
- **Verification (outside sandbox, per dotnet-build-policy):**
  - `dotnet restore project/OsEngine/OsEngine.csproj --nologo` -> success
  - `dotnet restore project/OsEngine.Tests/OsEngine.Tests.csproj --nologo` -> success
