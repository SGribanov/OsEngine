...
## Stabilization Update (2026-02-14) - Validate Strategy Name Before Start
### What changed
- Added early `Start(...)` guard in `OptimizerExecutor`:
  - `_master.StrategyName` must be non-empty/non-whitespace.
- Invalid configuration now aborts start before worker lifecycle begins, with explicit diagnostic log.
...
## Stabilization Update (2026-02-14) - Validate Settings Context Before Start
### What changed
- Added early `Start(...)` guard in `OptimizerExecutor`:
  - `_master.Settings` must be initialized.
- Missing settings now fail fast at start boundary with explicit diagnostic log.
...
## Stabilization Update (2026-02-14) - Guard Runtime Thread Count In Strategy Construction
### What changed
- Extended `GetInSampleOptimizationStrategy(...)` runtime validation:
  - `_master.ThreadsCount` must remain positive at strategy-construction time.
- On invalid runtime value, strategy creation is skipped with explicit diagnostic and returns `null`.
- Parallelism argument now uses validated `ThreadsCount` directly (no implicit `Math.Max` coercion).
...
## Stabilization Update (2026-02-14) - Reuse Shared Generic Enum Validation Helper
### What changed
- Added helper `IsDefinedEnum<TEnum>(TEnum value)` in `OptimizerExecutor`.
- Replaced repeated `Enum.IsDefined(...)` checks with helper across:
  - `Start(...)` config validation;
  - `PrimeThreadWorkerPlace()` runtime enum guards;
  - `GetInSampleOptimizationStrategy(...)` runtime strategy-settings guard.
...
## Stabilization Update (2026-02-14) - Extract Prime Worker Strategy Snapshot Guard Helper
### What changed
- Added helper `ValidatePrimeWorkerStrategySnapshots(...)` in `OptimizerExecutor`.
- Moved strategy/parameter runtime snapshot guards from `PrimeThreadWorkerPlace()` into the helper:
  - strategy name presence;
  - non-negative iteration count;
  - positive thread count;
  - parameter snapshots availability;
  - flags/params count match;
  - null-parameter entry detection.
- Removed duplicated inline validation block in `PrimeThreadWorkerPlace()` after extraction.
