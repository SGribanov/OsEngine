...
|---|---|---|---|---|---|---|---|
| TradeGrid runtime | `OsTrader/Grids/TradeGrid.cs` hot query/cancel paths | `P1` | High | In Progress | `tradegrid_query_collections_hotpath` | `#1017 -> #1022` (median): `9231.49 -> 8514.82 ns/op`, `1856.01 -> 1312.01 bytes/op` | Continue internal non-alloc chain (`GetOrdersBadPriceToGrid`, grid-hole helpers, cancel staging) with median latency non-regression gate. |
| Optimizer cache runtime | `Indicators/Aindicator.cs`, `OsOptimizer/OptEntity/IndicatorCache.cs`, `OptimizerMethodCache.cs` | `P2` | High | Planned | `indicator_cache_hit_path` | current median snapshot: `5653.00 ns/op`, `12952.02 bytes/op` (`#1022` run) | Reduce clone/key overhead, keep deterministic isolation guarantees. |
