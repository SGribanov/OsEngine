...
|---|---|---|---|---|---|---|---|
| TradeGrid runtime | `OsTrader/Grids/TradeGrid.cs` hot query/cancel paths | `P1` | High | In Progress | `tradegrid_query_collections_hotpath` | `#1017 -> #1023` (median): `9231.49 -> 8094.29 ns/op`, `1856.01 -> 992.01 bytes/op` | Continue internal non-alloc chain (`GetOrdersBadPriceToGrid`, grid-hole helpers, cancel staging) with median latency non-regression gate. |
| Optimizer cache runtime | `Indicators/Aindicator.cs`, `OsOptimizer/OptEntity/IndicatorCache.cs`, `OptimizerMethodCache.cs` | `P2` | High | In Progress | `indicator_cache_hit_path`, `optimizer_method_cache_hit_path`, `optimizer_cache_key_build_path` | `indicator #1017 -> #1027`: `4764.65 -> 1900.40 ns/op`, `12952.02 -> 448.02 bytes/op`; `method #1025 -> #1027`: `211.55 -> 148.82 ns/op`, `0.01 -> 0.01 bytes/op`; `key-build #1027`: `324.73 ns/op`, `0.01 bytes/op` | Continue P2 on key-build path (source-id token, result-type name, parameter-hash) and preserve deterministic isolation boundaries. |
