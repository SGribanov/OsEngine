...
public class MacdHistogramPersistenceTests
{
    [Fact]
    public void Save_ShouldPersistJson_AndLoadRoundTrip()
    {
        const string name = "CodexMacdHistogramJson";
        using MacdHistogramFileScope scope = new MacdHistogramFileScope(name);

        MacdHistogram source = new MacdHistogram(name, canDelete: true)
        {
            ColorUp = Color.DeepSkyBlue,
            ColorDown = Color.Maroon,
            PaintOn = false
        };
        source.Save();
        ...
        MacdHistogram loaded = new MacdHistogram(name, canDelete: true);
    ...
    {
        const string name = "CodexMacdHistogramLegacy";
        using MacdHistogramFileScope scope = new MacdHistogramFileScope(name);
    ...
    private sealed class MacdHistogramFileScope : IDisposable
    {
        private readonly string _engineDirPath;
        private readonly bool _engineDirExisted;
        private readonly List<FileState> _states;

        public MacdHistogramFileScope(string name)
        {
