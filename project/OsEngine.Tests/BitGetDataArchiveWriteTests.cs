...
public class BitGetDataArchiveWriteTests
{
    [Fact]
    public void WriteTempArchiveBytes_ShouldPersistBytes_AndReleaseFileHandle()
    {
        string root = Path.Combine(Path.GetTempPath(), "osengine-bitget-archive-" + Guid.NewGuid());
    ...
    {
        MethodInfo method = typeof(BitGetDataServerRealization).GetMethod(
