...
public class MoexFixFastCurrencyLogWriterTests
{
    [Fact]
    public void CreatePersistentLogWriter_ShouldCreateDirectory_TruncateNonAppendFile_AndUseCurrentLineShapeWithoutBom()
    {
        string root = Path.Combine(Path.GetTempPath(), "osengine-moexcurrency-log-" + Guid.NewGuid());
    ...
    {
        string root = Path.Combine(Path.GetTempPath(), "osengine-moexcurrency-log-" + Guid.NewGuid());
        string path = Path.Combine(root, "IncomingMFIX.log");
        MethodInfo createMethod = GetPrivateMethod("CreatePersistentLogWriter");
        MethodInfo formatMethod = GetPrivateMethod("FormatPersistentLogLine");
        ...
        try
        {
            ...
            string firstLine = (string)formatMethod.Invoke(null, new object[] { "first-fix", new DateTime(2026, 3, 8, 9, 15, 30) })!;
            string secondLine = (string)formatMethod.Invoke(null, new object[] { "second-fix", new DateTime(2026, 3, 8, 9, 15, 31) })!;
    ...
    {
        return typeof(MoexFixFastCurrencyServerRealization).GetMethod(methodName, BindingFlags.NonPublic | BindingFlags.Static)
