    ...
    public class AlertToChart:IIAlert
    {
        ...
        private static AlertToChartSettingsDto ParseLegacySettings(string content)
        {
            ...
            return new AlertToChartSettingsDto
            {
                ...
                BorderWidth = rows.Length > 4 ? Convert.ToInt32(rows[4], CultureInfo.InvariantCulture) : 0,
                ...
                ColorLineArgb = rows.Length > 8 ? Convert.ToInt32(rows[8], CultureInfo.InvariantCulture) : Color.Black.ToArgb(),
                ColorLabelArgb = rows.Length > 9 ? Convert.ToInt32(rows[9], CultureInfo.InvariantCulture) : Color.Black.ToArgb(),
                Music = music,
                SignalType = signalType,
                VolumeReaction = rows.Length > 12 ? rows[12].ToDecimal() : 0,
                Slippage = rows.Length > 13 ? rows[13].ToDecimal() : 0,
                NumberClosePosition = rows.Length > 14 ? Convert.ToInt32(rows[14], CultureInfo.InvariantCulture) : 0,
                OrderPriceType = orderPriceType,
                SlippageType = slippageType
            };
        }
