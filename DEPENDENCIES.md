...
This file tracks binary dependencies currently referenced from `project/OsEngine/OsEngine.csproj` via `HintPath` (`bin\Debug\*.dll`), their provenance, and migration status for Stage 2 / Step 4.3.
...
| `LiteDB.dll` | `5.0.19.0` | `project/OsEngine/bin/Debug/LiteDB.dll` | `032548393720FC372BD5FD25EE755D6080975E3B27BB94DC9A5BF3DDF2F41C5F` | NuGet candidate (`LiteDB` `5.0.19`), migration deferred until online restore is available |
...
`dotnet restore` currently cannot reach `https://api.nuget.org/v3/index.json` in this environment (`NU1301`, SSL/authentication chain issue), so package migrations cannot be verified safely here.

## Planned Migration Order (when online restore works)

1. `LiteDB` (`5.0.19`) via `<PackageReference>` with no API changes expected.
2. `RestSharp` via `<PackageReference>` pinned to a compatible API version; run connector smoke checks after migration.
3. `Jayrock.Json` replacement analysis in code paths that still require `Jayrock.Json`.
4. Keep vendor/custom assemblies (`cgate_net64`, `MtApi5`, `MtClient`, `QuikSharp`, `FinamApi`, `TInvestApi`) as binary or switch to local project references in a separate decision.
