...
## 2026-02-24 - Step 2.2 (InvariantCulture in persistence/protocol) - Incremental Adoption #537

- Hardened MOEX FIX heartbeat `TestRequest` payload formatting with explicit invariant culture:
  - `project/OsEngine/Market/Servers/MoexFixFastTwimeFutures/MoexFixFastTwimeFuturesServer.cs`
  - `project/OsEngine/Market/Servers/MoexFixFastCurrency/MoexFixFastCurrencyServer.cs`
- Replacements:
  - `DateTime.UtcNow.ToString("OsEngine")` -> `DateTime.UtcNow.ToString("OsEngine", CultureInfo.InvariantCulture)`
- Scope:
  - protocol string formatting determinism only; heartbeat/test-request behavior unchanged.
...
## 2026-02-24 - Step 2.2 (CultureInfo.InvariantCulture) - UI int parsing hardening (Bots/Connectors)
...
- Changes:
  ...
  - applied to UI textbox/config value parsing.
- Scope:
  - parser hardening only
  - connector/robot behavior unchanged for valid payloads.
...
## 2026-02-24 - Step 2.2 (CultureInfo.InvariantCulture) - UI/settings int parsing hardening (News/Entity dialogs)
...
- Changes:
  - replaced string-based int conversions:
    - `Convert.ToInt32(value)` -> `Convert.ToInt32(value, CultureInfo.InvariantCulture)`
  - applied to:
    - connector news settings textbox value
    - legacy connector settings load (`lines[2]`)
    - date-time dialog hour/min/sec textboxes
    - security volume decimals textbox.
- Scope:
  - parser hardening only
...
## 2026-02-24 - Step 2.2 (CultureInfo.InvariantCulture) - BingX/ExMo/BitGet int parsing hardening
...
## 2026-02-24 - Step 2.2 (CultureInfo.InvariantCulture) - CoinEx/KiteConnect/Mexc/XT int parsing hardening
...
- Changes:
  ...
  - applied to:
    ...
    - KiteConnect `userNumber` mapping to `NumberUser`
...
## 2026-02-24 - Step 2.2 (CultureInfo.InvariantCulture) - Binance/Bybit/BitMart/Transaq/QuikLua int parsing hardening
...
- Changes:
  ...
  - applied to:
    ...
    - BitMart spot/futures `client_order_id`, `clientOrderId`, and side/action mapping from API payload.
...
## 2026-02-24 - Step 2.2 (CultureInfo.InvariantCulture) - Bitfinex/Moex/IB/Plaza int parsing hardening
...
- Changes:
  - replaced string-based int conversions:
    - `Convert.ToInt32(value)` -> `Convert.ToInt32(value, CultureInfo.InvariantCulture)`
  - applied to:
    - Bitfinex websocket channel ids and order/user id mapping fields.
    - MoexFixFastTwimeFutures security id conversions from name/id mappings.
    - InteractiveBrokers `NumberMarket` lookup conversion.
    - Plaza security `NameId` lookup conversion.
- Scope:
  - parser hardening only
  - connector behavior unchanged for valid payloads.
...
## 2026-02-24 - Step 2.2 (CultureInfo.InvariantCulture) - MOEX FIX int.Parse hardening (Spot/Currency/Twime)

- Standardized string `int.Parse(...)` with explicit invariant culture in:
  - `project/OsEngine/Market/Servers/MoexFixFastSpot/MoexFixFastSpotServer.cs`
  - `project/OsEngine/Market/Servers/MoexFixFastCurrency/MoexFixFastCurrencyServer.cs`
  - `project/OsEngine/Market/Servers/MoexFixFastTwimeFutures/MoexFixFastTwimeFuturesServer.cs`
- Changes:
  ...
  - applied to:
    - TCP/FIX ports from XML config.
    - FIX fields (`SessionStatus`, `SecondaryClOrdID`).
...
## 2026-02-24 - Step 2.2 (CultureInfo.InvariantCulture) - Timestamp/Funding int.Parse hardening
...
- Changes:
  - replaced parsing:
    - `int.Parse(value)` -> `int.Parse(value, CultureInfo.InvariantCulture)`
  - applied to:
    - external timestamps from payload strings.
    - funding interval fields (`Hours`, `funding_interval`, `fundingRateGranularity`, etc.).
    - BinanceData regex/date-part parsing for archive month/date extraction.
- Scope:
  - parser hardening only
  - connector behavior unchanged for valid payloads.
...
## 2026-02-24 - Step 2.2 (CultureInfo.InvariantCulture) - Alor string int parsing hardening

- Standardized string-based int parsing with explicit invariant culture in:
  - `project/OsEngine/Market/Servers/Alor/AlorServer.cs`
- Changes:
  - replaced parsing:
    - `Convert.ToInt32(value)` -> `Convert.ToInt32(value, CultureInfo.InvariantCulture)`
  - applied to:
    ...
    - `baseMessage.comment` -> `order.NumberUser`.
    - parsed date/time fragments in helper conversion path (`year/month/day/hour/minute/second/ms`).
- Scope:
  - parser hardening only
  - connector behavior unchanged for valid payloads.
...
## 2026-02-24 - Step 2.2 (CultureInfo.InvariantCulture) - Finam/Atp/NinjaTrader int parsing hardening
...
- Changes:
  - Finam:
    - introduced single normalized market id:
      - `marketId = Convert.ToInt32(arrayMarkets[i], CultureInfo.InvariantCulture)`
    - replaced repeated `Convert.ToInt32(arrayMarkets[i])` comparisons with `marketId`.
  - Atp:
    - replaced string/int conversions in order-id and date/time parsing paths:
      - `Convert.ToInt32(value)` -> `Convert.ToInt32(value, CultureInfo.InvariantCulture)`.
  - NinjaTrader:
    - replaced `NumberUser` mapping from incoming string arrays:
      - `Convert.ToInt32(value)` -> `Convert.ToInt32(value, CultureInfo.InvariantCulture)`.
- Scope:
  - parser hardening only
  - connector behavior unchanged for valid payloads.
...
## 2026-02-24 - Step 2.2 (CultureInfo.InvariantCulture) - QuikLua/TraderNet string parsing hardening
...
- Changes:
  - QuikLua:
    - `Convert.ToInt32(value)` -> `Convert.ToInt32(value, CultureInfo.InvariantCulture)` for:
      - expiration date fragments from `exp` string.
      - `Scale`, `Qty`, `Flags`, `OpenInterest` fields from external payloads.
  - TraderNet:
    - `Convert.ToInt32(value)` -> `Convert.ToInt32(value, CultureInfo.InvariantCulture)` for:
      - `instr_type_c` security type parsing.
      - market-depth index fields (`del/ins/upd[*].k`).
- Scope:
  - parser hardening only
  - connector behavior unchanged for valid payloads.
...
## 2026-02-24 - Step 2.2 (CultureInfo.InvariantCulture) - AscendEX/BitGet/MoexAlgopack precision/funding hardening

- Standardized string-based numeric parsing with explicit invariant culture in:
  - `project/OsEngine/Market/Servers/AscendEX/AscendEXSpot/AscendexSpotServer.cs`
  - `project/OsEngine/Market/Servers/BitGet/BitGetFutures/BitGetServerFutures.cs`
  - `project/OsEngine/Market/Servers/MoexAlgopack/MoexAlgopackServer.cs`
- Changes:
  - AscendEX:
    - `Convert.ToInt32(priceScale/qtyScale)` -> `Convert.ToInt32(..., CultureInfo.InvariantCulture)`.
  - BitGet Futures:
    - `int.Parse(item.fundingRateInterval)` -> `int.Parse(item.fundingRateInterval, CultureInfo.InvariantCulture)`.
  - MoexAlgopack:
    - security decimal fields from response arrays:
      - `Convert.ToInt32(item[5|8])` -> `Convert.ToInt32(..., CultureInfo.InvariantCulture)`.
- Scope:
  - parser hardening only
  - connector behavior unchanged for valid payloads.
...
## 2026-02-24 - Step 2.2 (CultureInfo.InvariantCulture) - Optimizer/Tester settings parsing hardening

- Standardized string-based settings parsing with explicit invariant culture in:
  - `project/OsEngine/Market/Servers/Optimizer/OptimizerDataStorageUi.xaml.cs`
  - `project/OsEngine/Market/Servers/Optimizer/OptimizerDataStorage.cs`
  - `project/OsEngine/Market/Servers/Tester/TesterServerUi.xaml.cs`
  - `project/OsEngine/Market/Servers/Tester/TesterServer.cs`
- Changes:
  - replaced parsing in UI/settings flows:
  ...
  - replaced parsing from persisted arrays:
...
## 2026-02-24 - Step 2.2 (CultureInfo.InvariantCulture) - UI/params/string parsing hardening (mixed)

- Standardized remaining string-based parsing with explicit invariant culture in:
...
- Changes:
  - `Convert.ToInt32(value)` -> `Convert.ToInt32(value, CultureInfo.InvariantCulture)` for:
    - UI textbox/grid values (`TimeDelaySeconds`, `Decimals`, `DecimalsVolume`).
    - server parameter integer load from persisted value array.
    - MetaTrader5 `magic` -> `NumberUser`.
    - MOEX year-fragment conversion (`year.ToString().Substring(...)`).
  - `int.Parse(value)` -> `int.Parse(value, CultureInfo.InvariantCulture)` for:
    - Telegram news interval extraction from regex group.
- Scope:
  - parser hardening only
  - behavior unchanged for valid payloads.
...
## 2026-02-24 - Step 2.2 (CultureInfo.InvariantCulture) - AServerParameterUi string int parsing hardening

- Standardized string-based int parsing with explicit invariant culture in:
  - `project/OsEngine/Market/Servers/AServerParameterUi.xaml.cs`
- Changes:
  - replaced parameter-int UI parsing:
    - `Convert.ToInt32(str)` -> `Convert.ToInt32(str, CultureInfo.InvariantCulture)`.
- Scope:
  - parser hardening only
  - behavior unchanged for valid payloads.
...
## 2026-02-24 - Step 2.2 (CultureInfo.InvariantCulture) - OsDataSet string parsing hardening

- Standardized string-based parsing with explicit invariant culture in:
  - `project/OsEngine/OsData/OsDataSet.cs`
- Changes:
  - replaced string parsing in persisted settings/data paths:
...
## 2026-02-24 - Step 2.2 (CultureInfo.InvariantCulture) - OsData UI/string parsing hardening
...
- Changes:
  - replaced UI/text/grid parsing:
    - `Convert.ToInt32(value)` -> `Convert.ToInt32(value, CultureInfo.InvariantCulture)`
    - `int.Parse(value)` -> `int.Parse(value, CultureInfo.InvariantCulture)`
  - applied to:
    - `MarketDepthDepth` selection.
    - row/label index parsing in security/data-grid navigation.
    - duplication period parsing from combo box.
    - update hour parsing from text box.
    - grid row id parsing in dataset painter flows.
- Scope:
  - parser hardening only
  - behavior unchanged for valid payloads.
...
## 2026-02-24 - Step 2.2 (CultureInfo.InvariantCulture) - Strategy/Journal/TradeGrid pair settings parsing hardening
...
- Standardized string-based parsing with explicit invariant culture in:
  ...
  - `project/OsEngine/OsTrader/Panels/Tab/BotTabPair.cs`
- Changes:
  - replaced parsing in persisted settings and UI/grid mapping flows:
    - `Convert.ToInt32(value)` -> `Convert.ToInt32(value, CultureInfo.InvariantCulture)`
  - applied to:
    - strategy parameter load (`save[]` / time arrays).
    - journal row-number reads from grid cells.
    - trade-grid settings load from `values[]`.
    - pair-tab settings load (`lines[]`) and grid row index reads.
- Scope:
  - parser hardening only
  - behavior unchanged for valid payloads.
...
## 2026-02-24 - Step 2.2 (CultureInfo.InvariantCulture) - GlobalPositionViewer/SecuritiesUi parsing hardening

- Standardized string-based parsing with explicit invariant culture in:
  - `project/OsEngine/OsTrader/GlobalPositionViewer.cs`
  - `project/OsEngine/Entity/SecuritiesUi.xaml.cs`
- Changes:
  - replaced UI/grid parsing:
    - `Convert.ToInt32(value)` -> `Convert.ToInt32(value, CultureInfo.InvariantCulture)`
  - applied to:
    - row-number sorting/selection and selected position ids in global position viewer.
    - decimal precision and result navigation label values in securities UI.
- Scope:
  - parser hardening only
  - behavior unchanged for valid payloads.
...
## 2026-02-24 - Step 2.2 (CultureInfo.InvariantCulture) - Journal UI grid/string parsing hardening

- Standardized string-based parsing with explicit invariant culture in:
  - `project/OsEngine/Journal/JournalUi.xaml.cs`
  - `project/OsEngine/Journal/JournalUi2.xaml.cs`
- Changes:
  - replaced parsing of string/grid-based values:
    - `Convert.ToInt32(value)` -> `Convert.ToInt32(value, CultureInfo.InvariantCulture)`
  - applied to:
    - selected/open/close position ids from data-grid cells.
    - range boundaries parsed from `selectNum/selectNums` split strings.
    - lists of selected position numbers.
- Scope:
  - parser hardening only
  - behavior unchanged for valid payloads.
...
## 2026-02-24 - Step 2.2 (CultureInfo.InvariantCulture) - TradeGrid UI string parsing hardening
...
- Changes:
  - replaced UI/grid parsing:
    - `Convert.ToInt32(value)` -> `Convert.ToInt32(value, CultureInfo.InvariantCulture)`
  - applied to:
    - text box settings (`Fail*`, `WaitSeconds`, `LineCountStart`, `StopBy*`, `AutoStarter*`, `Max*`, `DelayInReal`).
    - data-grid cell parsing for row number values.
- Scope:
  - parser hardening only
  - behavior unchanged for valid payloads.
...
## 2026-02-24 - Step 2.2 (CultureInfo.InvariantCulture) - OptimizerUi string parsing hardening
...
- Changes:
  - replaced UI/grid parsing:
    - `Convert.ToInt32(value)` -> `Convert.ToInt32(value, CultureInfo.InvariantCulture)`
  - applied to:
    - optimizer iteration/filter/start-deposit text fields.
    - Bayesian optimization integer parameter text fields.
    - integer parameter values from optimizer parameter grid cells.
  - kept non-string computed numeric casts (`sortedValue.Count * ...`) unchanged.
- Scope:
  - parser hardening only
  - behavior unchanged for valid payloads.
...
## 2026-02-24 - Step 2.2 (CultureInfo.InvariantCulture) - OsTrader internal/grid settings parsing hardening
...
- Changes:
  - replaced parsing in text/split/settings paths:
    - `Convert.ToInt32(value)` -> `Convert.ToInt32(value, CultureInfo.InvariantCulture)`
  - applied to:
    - open-position lifetime/time parsing from UI text and `openTimeStr`.
    - manual control seconds fields.
    - grid stop/autostart/errors-reaction settings loaded from `values[]`.
- Scope:
  - parser hardening only
  - behavior unchanged for valid payloads.
...
## 2026-02-24 - Step 2.2 (CultureInfo.InvariantCulture) - Optimizer settings/master and BotTabIndex parsing hardening
...
- Changes:
  - OptimizerSettings:
    - `Convert.ToInt32(reader.ReadLine() ?? "...")` -> `Convert.ToInt32(..., CultureInfo.InvariantCulture)`.
  - OptimizerMaster:
    - string-based `str[]` conversions for persisted values:
      - `Convert.ToInt32(str[i])` -> `Convert.ToInt32(..., CultureInfo.InvariantCulture)`.
    - kept non-string computed `TotalDays` casts unchanged.
  - BotTabIndex:
    - `Convert.ToInt32(val*.Split('A')[1])` -> `Convert.ToInt32(..., CultureInfo.InvariantCulture)`.
- Scope:
  - parser hardening only
  - behavior unchanged for valid payloads.
...
## 2026-02-23 - Step 2.2 (CultureInfo.InvariantCulture) - Timestamp parsing hardening (Bitfinex Spot/Futures)
...
- Changes:
  - replaced timestamp conversions:
    - `Convert.ToInt64(value)` -> `Convert.ToInt64(value, CultureInfo.InvariantCulture)`
  - applied in candles/trades/order/myTrade/history timestamp paths.
- Scope:
  - parser hardening only
  - connector behavior unchanged for valid payloads.
...
## 2026-02-23 - Step 2.2 (CultureInfo.InvariantCulture) - Timestamp parsing hardening (Bybit)
...
## 2026-02-23 - Step 2.2 (CultureInfo.InvariantCulture) - Timestamp parsing hardening (ExMo Spot)
...
## 2026-02-23 - Step 2.2 (CultureInfo.InvariantCulture) - Timestamp parsing hardening (Mexc Spot)
...
- Changes:
  - replaced timestamp conversions:
    - `Convert.ToInt64(value)` -> `Convert.ToInt64(value, CultureInfo.InvariantCulture)`
  - applied in trades/depth/order/myTrade timestamp paths.
- Scope:
  - parser hardening only
  - connector behavior unchanged for valid payloads.
...
## 2026-02-23 - Step 2.2 (CultureInfo.InvariantCulture) - Timestamp parsing hardening (GateIo Spot)
...
## 2026-02-23 - Step 2.2 (CultureInfo.InvariantCulture) - Timestamp parsing hardening (GateIo Futures)
...
- Changes:
  - replaced timestamp conversions:
    - `Convert.ToInt64(value)` -> `Convert.ToInt64(value, CultureInfo.InvariantCulture)`
    - `long.Parse(value)` -> `long.Parse(value, CultureInfo.InvariantCulture)`
  - applied in candles/trades/depth/order/myTrade/funding timestamp paths.
- Scope:
  - parser hardening only
  - connector behavior unchanged for valid payloads.
...
## 2026-02-23 - Step 2.2 (CultureInfo.InvariantCulture) - Timestamp parsing hardening (HTX Spot)
...
## 2026-02-23 - Step 2.2 (CultureInfo.InvariantCulture) - Timestamp parsing hardening (HTX Futures)
...
## 2026-02-23 - Step 2.2 (CultureInfo.InvariantCulture) - Timestamp parsing hardening (OKX Expiration)
...
- Changes:
  - replaced timestamp conversions:
    - `long.Parse(value)` -> `long.Parse(value, CultureInfo.InvariantCulture)`
  - applied in instrument expiration timestamp paths.
- Scope:
  - parser hardening only
  - connector behavior unchanged for valid payloads.
...
## 2026-02-23 - Step 2.2 (CultureInfo.InvariantCulture) - Timestamp parsing hardening (BitGet Spot/Futures candles)
...
- Changes:
  - replaced timestamp conversions:
    - `long.Parse(value)` -> `long.Parse(value, CultureInfo.InvariantCulture)`
  - applied in candle timestamp paths.
- Scope:
  - parser hardening only
  - connector behavior unchanged for valid payloads.
...
## 2026-02-23 - Step 2.2 (CultureInfo.InvariantCulture) - Timestamp parsing hardening (GateIo Data)
...
- Changes:
  - replaced timestamp conversions:
    - `long.Parse(value)` -> `long.Parse(value, CultureInfo.InvariantCulture)`
  - applied in historical trades data timestamp paths.
- Scope:
  - parser hardening only
  - connector behavior unchanged for valid payloads.
...
## 2026-02-23 - Step 2.2 (CultureInfo.InvariantCulture) - Timestamp parsing hardening (BitGet Data)
...
- Changes:
  - replaced timestamp conversions:
    - `long.Parse(value)` -> `long.Parse(value, CultureInfo.InvariantCulture)`
  - applied in market trade timestamp path.
- Scope:
  - parser hardening only
  - connector behavior unchanged for valid payloads.
...
## 2026-02-23 - Step 2.2 (CultureInfo.InvariantCulture) - Timestamp parsing hardening (Polygon)
...
- Changes:
  - replaced timestamp conversions:
    - `long.Parse(value)` -> `long.Parse(value, CultureInfo.InvariantCulture)`
  - applied in candle timestamp path.
- Scope:
  - parser hardening only
  - connector behavior unchanged for valid payloads.
...
## 2026-02-23 - Step 2.2 (CultureInfo.InvariantCulture) - Timestamp parsing hardening (Core/Yahoo/TraderNet)
...
- Changes:
  - replaced timestamp conversions:
    - `long.Parse(value)` -> `long.Parse(value, CultureInfo.InvariantCulture)`
    - `Convert.ToInt64(value)` -> `Convert.ToInt64(value, CultureInfo.InvariantCulture)`
  - applied in additional market data time, candle timestamp paths.
- Scope:
  - parser hardening only
  - connector behavior unchanged for valid payloads.
...
## 2026-02-23 - Step 2.2 (CultureInfo.InvariantCulture) - Timestamp parsing hardening (Deribit)
...
- Changes:
  - replaced timestamp conversions:
    - `long.Parse(value)` -> `long.Parse(value, CultureInfo.InvariantCulture)`
    - `Convert.ToInt64(value)` -> `Convert.ToInt64(value, CultureInfo.InvariantCulture)`
  - applied in expiration, candles, trades, myTrades and orders timestamp paths.
- Scope:
  - parser hardening only
  - connector behavior unchanged for valid payloads.
...
## 2026-02-23 - Step 2.2 (CultureInfo.InvariantCulture) - Timestamp parsing hardening (Data connectors)
...
- Changes:
  - replaced timestamp conversions:
    - `long.Parse(value)` -> `long.Parse(value, CultureInfo.InvariantCulture)`
  - applied in `DateTimeOffset.FromUnixTimeMilliseconds(...)` timestamp paths for candles/trades.
- Scope:
  - parser hardening only
  - connector behavior unchanged for valid payloads.
...
## 2026-02-23 - Step 2.2 (CultureInfo.InvariantCulture) - ID and request timestamp normalization (Binance/Quik/Plaza/HTX)

- Standardized numeric string parsing and formatting with explicit invariant culture in:
  - `project/OsEngine/Market/Servers/Binance/Spot/BinanceServerSpot.cs`
  - `project/OsEngine/Market/Servers/Binance/Futures/BinanceServerFutures.cs`
  - `project/OsEngine/Market/Servers/QuikLua/QuikLuaServer.cs`
  - `project/OsEngine/Market/Servers/Plaza/PlazaServer.cs`
  - `project/OsEngine/Market/Servers/HTX/Futures/HTXFuturesServer.cs`
- Changes:
  - replaced conversion of external string ids:
    - `Convert.ToInt64(value)` -> `Convert.ToInt64(value, CultureInfo.InvariantCulture)`
  - standardized outbound timestamp/id string formatting for Binance REST params:
    - `.ToString()` -> `.ToString(CultureInfo.InvariantCulture)`
- Scope:
  - parser/serialization hardening only
  - connector behavior unchanged for valid payloads.
...
## 2026-02-23 - Step 2.2 (CultureInfo.InvariantCulture) - FED rates timestamp parsing hardening (OsData)

- Standardized numeric string parsing with explicit invariant culture in:
  - `project/OsEngine/OsData/LqdtDataFakeServer.cs`
- Changes:
  - replaced parsing of external FED rates timestamp:
    - `Convert.ToInt64(value)` -> `Convert.ToInt64(value, CultureInfo.InvariantCulture)`
- Scope:
  - parser hardening only
  - behavior unchanged for valid payloads.
...
## 2026-02-23 - Step 2.2 (CultureInfo.InvariantCulture) - MOEX FIX int parsing hardening (ports/order user ids)

- Standardized numeric string parsing with explicit invariant culture in:
  - `project/OsEngine/Market/Servers/MoexFixFastCurrency/MoexFixFastCurrencyServer.cs`
  - `project/OsEngine/Market/Servers/MoexFixFastTwimeFutures/MoexFixFastTwimeFuturesServer.cs`
- Changes:
  - replaced string-based int conversions in connector configs and FIX order-id mapping:
    - `Convert.ToInt32(value)` -> `Convert.ToInt32(value, CultureInfo.InvariantCulture)`
  - applied to:
    - XML `port` values from multicast/recovery configs
    - `origClOrdId` / `NumUserInSystem` / FIX tag `41` conversions to `order.NumberUser`.
- Scope:
  - parser hardening only
  - connector behavior unchanged for valid payloads.
...
## 2026-02-23 - Step 2.2 (CultureInfo.InvariantCulture) - GateIo user-id int parsing hardening (Spot/Futures)

- Standardized numeric string parsing with explicit invariant culture in:
  - `project/OsEngine/Market/Servers/GateIo/GateIoSpot/GateIoServerSpot.cs`
  - `project/OsEngine/Market/Servers/GateIo/GateIoFutures/GateIoServerFutures.cs`
- Changes:
  - replaced user id parsing from exchange text tags:
    - `Convert.ToInt32(value)` -> `Convert.ToInt32(value, CultureInfo.InvariantCulture)`
  - applied to values parsed from `text.Replace("t-", "")` in orders/my-trades paths.
- Scope:
  - parser hardening only
  - connector behavior unchanged for valid payloads.
...
## 2026-02-23 - Step 2.2 (CultureInfo.InvariantCulture) - User-id int parsing hardening (OKX/Woo/Deribit)
...
- Standardized numeric string parsing with explicit invariant culture in:
  ...
  - `project/OsEngine/Market/Servers/Deribit/DeribitServer.cs`
- Changes:
  - replaced user label/client-order-id parsing:
    - `Convert.ToInt32(value)` -> `Convert.ToInt32(value, CultureInfo.InvariantCulture)`
  - applied to:
    - OKX `clOrdId`
    - Woo `cid` / `clientOrderId`
    - Deribit `label`.
- Scope:
  - parser hardening only
  - connector behavior unchanged for valid payloads.
...
## 2026-02-23 - Step 2.2 (CultureInfo.InvariantCulture) - KuCoin/Pionex int parsing hardening
...
- Changes:
  - replaced string-based int conversions:
    - `Convert.ToInt32(value)` -> `Convert.ToInt32(value, CultureInfo.InvariantCulture)`
  - applied to:
    - KuCoin `clientOid` in active/open/history order flows
    - Pionex `quotePrecision` / `basePrecision`
    - Pionex `clientOrderId` in order mapping paths.
- Scope:
  - parser hardening only
  - connector behavior unchanged for valid payloads.
...
## 2026-02-24 - Step 2.2 (CultureInfo.InvariantCulture) - Alerts UI/settings int parsing hardening
...
- Changes:
  - replaced string-based int conversions:
    - `Convert.ToInt32(value)` -> `Convert.ToInt32(value, CultureInfo.InvariantCulture)`
  - applied to alert textbox values and legacy file settings (`lines[]`/`rows[]`).
- Scope:
  - parser hardening only
  - runtime behavior unchanged for valid payloads.
...
## 2026-02-24 - Step 2.2 (CultureInfo.InvariantCulture) - Proxy settings/UI int parsing hardening
...
- Changes:
  - replaced string-based int conversions:
    - `Convert.ToInt32(value)` and `int.Parse(value)` -> `Convert.ToInt32(value, CultureInfo.InvariantCulture)`
  - applied to serialized proxy values (`Split('%')`, legacy settings `lines[]`) and UI/grid values.
- Scope:
  - parser hardening only
  - runtime behavior unchanged for valid payloads.
...
## 2026-02-24 - Step 2.2 (CultureInfo.InvariantCulture) - Entity persistence int parsing hardening
...
- Changes:
  - replaced string-based int conversions:
    - `Convert.ToInt32(value)` -> `Convert.ToInt32(value, CultureInfo.InvariantCulture)`
  - replaced legacy parsing:
    - `int.TryParse(value, out var x)` -> `int.TryParse(value, NumberStyles.Integer, CultureInfo.InvariantCulture, out var x)`
  - applied to persisted string arrays (`saveArray/arraySave/savStr/lines`).
- Scope:
  - parser hardening only
  - runtime behavior unchanged for valid payloads.
...
## 2026-02-24 - Step 2.2 (CultureInfo.InvariantCulture) - SystemAnalyze/Optimizer/Indicator/Depth int parsing hardening
...
- Changes:
  - replaced string-based int conversions:
    - `Convert.ToInt32(value)` -> `Convert.ToInt32(value, CultureInfo.InvariantCulture)`
  - applied to UI textboxes and persisted split-array values.
- Scope:
  - parser hardening only
  - runtime behavior unchanged for valid payloads.
...
## 2026-02-24 - Step 2.2 (CultureInfo.InvariantCulture) - AutoFollow settings/UI int parsing hardening
...
- Changes:
  - replaced string-based int conversions:
    - `Convert.ToInt32(value)` -> `Convert.ToInt32(value, CultureInfo.InvariantCulture)`
  - applied to legacy settings parsing (`save[]/lines[]`) and UI/grid values.
- Scope:
  - parser hardening only
  - runtime behavior unchanged for valid payloads.
...
## 2026-02-24 - Step 2.2 (CultureInfo.InvariantCulture) - BotTab and AutoTest UI int parsing hardening
...
- Changes:
  - replaced string-based int conversions:
    - `Convert.ToInt32(value)` -> `Convert.ToInt32(value, CultureInfo.InvariantCulture)`
  - replaced validation parsing in test-bot params:
    - `Int32.TryParse(value, out ...)` -> `Int32.TryParse(value, NumberStyles.Integer, CultureInfo.InvariantCulture, out ...)`
- Scope:
  - parser hardening only
  - runtime behavior unchanged for valid payloads.
...
## 2026-02-24 - Step 2.2 (CultureInfo.InvariantCulture) - Robots settings/UI int parsing hardening
...
## 2026-02-26 - Step 0.3 (silent-catch visibility) - GateIo Spot/Futures catch logging hardening block
...
- Changes:
  - bare `catch { // ignore }` converted to `catch (Exception ex)`.
  - added `SendLogMessage(ex.ToString(), LogMessageType.Error)` in socket-dispose, unsubscribe and order-id parse branches.
...
## 2026-02-26 - Step 0.3 (silent-catch visibility) - Bitfinex Spot/Futures catch logging hardening block
...
- Changes:
  - bare `catch { // ignore }` converted to `catch (Exception ex)`.
  - added `SendLogMessage(ex.ToString(), LogMessageType.Error)` in websocket dispose/unsubscribe and order-id parse branches.
  - existing branch control flow preserved (`continue/break/return`).
- Scope:
  - observability-only hardening
  - no behavior changes on successful paths.
...
## 2026-02-26 - Step 0.3 (silent-catch visibility) - KuCoin Spot/Futures catch logging hardening block
...
## 2026-02-26 - Step 0.3 (silent-catch visibility) - XT Spot/Futures catch logging hardening block
...
## 2026-02-26 - Step 0.3 (silent-catch visibility) - BitMart Spot/Futures catch logging hardening block
...
## 2026-02-26 - Step 0.3 (silent-catch visibility) - BitGet Spot/Futures catch logging hardening block
...
## 2026-02-26 - Step 0.3 (silent-catch visibility) - BingX Futures + BloFin Futures catch logging hardening block
...
## 2026-02-26 - Step 0.3 (silent-catch visibility) - OKX + Pionex Spot catch logging hardening block
...
## 2026-02-26 - Step 0.3 (silent-catch visibility) - HTX Swap + InteractiveBrokers catch logging hardening block
...
- Changes:
  - bare `catch { // ignore }` converted to `catch (Exception ex)`.
  - added `SendLogMessage(ex.ToString(), LogMessageType.Error)` in socket dispose and parse/decode branches.
  - existing branch control flow preserved (`continue/break/return`).
- Scope:
  - observability-only hardening
  - no behavior changes on successful paths.
...
## 2026-02-26 - Step 0.3 (silent-catch visibility) - Woo + Alor catch logging hardening block
...
## 2026-02-26 - Step 0.3 (silent-catch visibility) - Bybit + AscendEX Spot catch logging hardening block
...
- Changes:
  - bare `catch { // ignore }` converted to `catch (Exception ex)`.
  - added `SendLogMessage(ex.ToString(), LogMessageType.Error)` in parse/socket branches.
  - existing branch control flow preserved (`continue/break/return`).
- Scope:
  - observability-only hardening
  - no behavior changes on successful paths.
...
## 2026-02-26 - Step 0.3 (silent-catch visibility) - Deribit + Mexc Spot catch logging hardening block

- Added explicit exception visibility for previously silent catch blocks in:
  - `project/OsEngine/Market/Servers/Deribit/DeribitServer.cs`
  - `project/OsEngine/Market/Servers/Mexc/MexcSpot/MexcSpotServer.cs`
- Changes:
  - bare `catch` blocks with `// ignore` converted to `catch (Exception ex)`.
  - added `SendLogMessage(ex.ToString(), LogMessageType.Error)` in socket/parse branches.
  - existing branch control flow preserved.
- Scope:
  - observability-only hardening
  - no behavior changes on successful paths.
...
## 2026-02-26 - Step 0.3 (silent-catch visibility) - KiteConnect + InteractiveBrokersServer catch logging hardening block

- Added explicit exception visibility for previously silent catch blocks in:
  - `project/OsEngine/Market/Servers/KiteConnect/KiteConnectServer.cs`
  - `project/OsEngine/Market/Servers/InteractiveBrokers/InteractiveBrokersServer.cs`
- Changes:
  - bare `catch` blocks with comments `// ignore` / `// ignored` converted to `catch (Exception ex)`.
  - added `SendLogMessage(ex.ToString(), LogMessageType.Error)` in affected branches.
  - existing branch control flow preserved.
- Scope:
  - observability-only hardening
  - no behavior changes on successful paths.
...
## 2026-02-26 - Step 0.3 (silent-catch visibility) - ComparePositionsModule + AtpServer catch logging hardening block
...
## 2026-02-26 - Step 0.3 (silent-catch visibility) - Binance Spot/Futures catch logging hardening follow-up

- Added explicit exception visibility for remaining bare catches in:
  - `project/OsEngine/Market/Servers/Binance/Spot/BinanceServerSpot.cs`
  - `project/OsEngine/Market/Servers/Binance/Futures/BinanceServerFutures.cs`
- Changes:
  - `catch` -> `catch (Exception ex)` in connect and trade-history paths.
  - parse-fallback branch now includes parse exception details in error payload.
  - static listen-key parse guard now traces unexpected exceptions via `Trace.TraceWarning(ex.ToString())`.
  - preserved existing return/control-flow behavior.
- Scope:
  - observability-only hardening
  - no behavior changes on successful paths.
...
## 2026-02-26 - Step 0.3 (silent-catch visibility) - BingX Spot/Futures catch logging hardening follow-up

- Added explicit exception visibility for remaining bare catches in:
  - `project/OsEngine/Market/Servers/BingX/BingXSpot/BingXServerSpot.cs`
  - `project/OsEngine/Market/Servers/BingX/BingXFutures/BingXServerFutures.cs`
- Changes:
  - `catch` -> `catch (Exception ex)` in decompression, listen-key renew and candle-history parse fallback paths.
  - parse fallback now includes parse exception details in log message.
  - preserved existing return/control-flow behavior.
- Scope:
  - observability-only hardening
  - no behavior changes on successful paths.
...
## 2026-02-26 - Step 0.3 (silent-catch visibility) - BitMart Spot/Futures catch logging hardening follow-up

- Added explicit exception visibility for remaining bare catches in:
  - `project/OsEngine/Market/Servers/BitMart/BitMartSpot/BitMartSpotServer.cs`
  - `project/OsEngine/Market/Servers/BitMart/BitMartFutures/BitMartFutures.cs`
- Changes:
  - `catch` -> `catch (Exception ex)` in client-order-id parsing paths.
  - added parse-failure details to connector log messages while preserving current fallback/return behavior.
  - preserved existing return/control-flow behavior.
- Scope:
  - observability-only hardening
  - no behavior changes on successful paths.
...
## 2026-02-26 - Step 0.3 (silent-catch visibility) - KuCoin Spot/Futures catch logging hardening follow-up

- Added explicit exception visibility for remaining bare catches in:
  - `project/OsEngine/Market/Servers/KuCoin/KuCoinSpot/KuCoinSpotServer.cs`
  - `project/OsEngine/Market/Servers/KuCoin/KuCoinFutures/KuCoinFuturesServer.cs`
- Changes:
  - `catch` -> `catch (Exception ex)` in `clientOid` parsing paths.
  - added parse-failure details to connector log messages while preserving current fallback/return behavior.
  - preserved existing return/control-flow behavior.
- Scope:
  - observability-only hardening
...
## 2026-02-26 - Step 0.3 (silent-catch visibility) - Bybit + Mexc catch logging hardening follow-up

- Added explicit exception visibility for remaining bare catches in:
  - `project/OsEngine/Market/Servers/Bybit/BybitServer.cs`
  - `project/OsEngine/Market/Servers/Mexc/MexcSpot/MexcSpotServer.cs`
- Changes:
  - `catch` -> `catch (Exception ex)` in portfolio/position parsing and client-order-id parsing paths.
  - for static portfolio factory path, added warning via `System.Diagnostics.Trace.TraceWarning(...)`.
...
## 2026-02-26 - Step 0.3 (silent-catch visibility) - ServerTickStorage+Tester+FinamGrpc catch logging hardening follow-up
...
- Added explicit exception visibility for remaining bare catches in:
  ...
  - `project/OsEngine/Market/Servers/FinamGrpc/FinamGrpcServer.cs`
- Changes:
  - `catch` -> `catch (Exception ex)` in file/trade parsing and stream `MoveNext` fallback paths.
  - routed diagnostics to existing logs (`SendNewLogMessage`/`SendLogMessage`) and trace warning in streaming loops.
  - preserved existing fallback/return control flow.
- Scope:
  - observability-only hardening
  - no behavior changes on successful paths.
...
## 2026-02-26 - Step 0.3 (silent-catch visibility) - TInvest+QuikLua+Transaq catch logging hardening follow-up
...
- Added explicit exception visibility for remaining bare catches in:
  ...
  - `project/OsEngine/Market/Servers/Transaq/TransaqServer.cs`
- Changes:
  - `catch` -> `catch (Exception ex)` in reconnect/dispose/portfolio fallback paths.
  - routed diagnostics via `System.Diagnostics.Trace.TraceWarning(...)` (TInvest) and existing connector log paths (QuikLua/Transaq).
  - preserved existing fallback/return control flow and non-throwing behavior.
- Scope:
  - observability-only hardening
  - no behavior changes on successful paths.
...
## 2026-02-26 - Step 0.3 (silent-catch visibility) - MoexFixFastSpot catch logging hardening block

- Added explicit exception visibility for remaining bare catches in:
  - `project/OsEngine/Market/Servers/MoexFixFastSpot/MoexFixFastSpotServer.cs`
- Changes:
  - replaced bare `catch` blocks in socket close/dispose and order-id parse fallback paths with explicit exception handling.
  - diagnostics routed to `System.Diagnostics.Trace.TraceWarning(ex.ToString())` to preserve runtime flow without throwing.
  - preserved existing reconnect/dispose and fallback logic.
- Scope:
  - observability-only hardening
  - no behavior changes on successful paths.
...
## 2026-02-26 - Step 0.3 (silent-catch visibility) - MoexFixFastCurrency+TwimeFutures catch logging hardening block

- Added explicit exception visibility for remaining bare catches in:
  - `project/OsEngine/Market/Servers/MoexFixFastCurrency/MoexFixFastCurrencyServer.cs`
  - `project/OsEngine/Market/Servers/MoexFixFastTwimeFutures/MoexFixFastTwimeFuturesServer.cs`
- Changes:
  - replaced bare `catch` blocks in socket close/dispose and snapshot/socket shutdown fallback paths with explicit exception handling.
  - diagnostics routed to `System.Diagnostics.Trace.TraceWarning(ex.ToString())` to preserve runtime flow without throwing.
  - preserved existing reconnect/dispose and fallback logic.
- Scope:
  - observability-only hardening
...
## 2026-02-26 - Step 2.2 (InvariantCulture) - Trade persistence parse coverage hardening block

- Expanded tests for `Trade` persistence parsing paths:
  - `project/OsEngine.Tests/TradeCoreTests.cs`
- Changes:
  - added IQFeed legacy RU datetime parse test with deterministic fallback input.
...
## 2026-02-27 - Step 2.2 (InvariantCulture) - Order malformed cancel-tail compatibility coverage block

- Expanded tests for `Order` persistence parsing paths:
  - `project/OsEngine.Tests/OrderPersistenceTests.cs`
- Changes:
  - added compatibility test for malformed cancel-info tail (`IsSendToCancel&CancellingTryCount&LastCancelTryLocalTime` block with invalid part count).
  - asserted parser fallback defaults when cancel-info tail is malformed.
- Scope:
  - test-only hardening
  - no production runtime behavior changes.
...
## 2026-02-27 - Step 2.2 (InvariantCulture) - Order legacy Patrial-state compatibility coverage block

- Expanded tests for `Order` persistence parsing paths:
  - `project/OsEngine.Tests/OrderPersistenceTests.cs`
- Changes:
  - added compatibility test for legacy misspelled state value (`Patrial`) in `SetOrderFromString()`.
  - asserted mapping to `OrderStateType.Partial` through enum alias support.
- Scope:
  - test-only hardening
  - no production runtime behavior changes.
...
- Host-context verification (outside sandbox):
  - `dotnet test project/OsEngine.Tests/OsEngine.Tests.csproj --no-restore --configuration Release --nologo` -> passed `429/429`

## 2026-02-27 - Step 2.2 (InvariantCulture) - Order legacy Activ-state compatibility coverage block

- Expanded tests for `Order` persistence parsing paths:
  - `project/OsEngine.Tests/OrderPersistenceTests.cs`
- Changes:
  - added compatibility test for legacy misspelled state value (`Activ`) in `SetOrderFromString()`.
  - asserted mapping to `OrderStateType.Active` through enum alias support.
- Scope:
  - test-only hardening
  - no production runtime behavior changes.
...
- Host-context verification (outside sandbox):
  - `dotnet test project/OsEngine.Tests/OsEngine.Tests.csproj --no-restore --configuration Release --nologo` -> passed `430/430`

## 2026-02-27 - Step 2.2 (InvariantCulture) - Order numeric-state payload compatibility coverage block

- Expanded tests for `Order` persistence parsing paths:
  - `project/OsEngine.Tests/OrderPersistenceTests.cs`
- Changes:
  - added compatibility test for legacy payload where order state is stored as numeric enum value (`"5"`).
  - asserted mapping to `OrderStateType.Partial` in `SetOrderFromString()`.
- Scope:
  - test-only hardening
  - no production runtime behavior changes.
...
- Host-context verification (outside sandbox):
  - `dotnet test project/OsEngine.Tests/OsEngine.Tests.csproj --no-restore --configuration Release --nologo` -> passed `431/431`

## 2026-02-27 - Step 2.2 (InvariantCulture) - Order numeric-active-state payload compatibility coverage block

- Expanded tests for `Order` persistence parsing paths:
  - `project/OsEngine.Tests/OrderPersistenceTests.cs`
- Changes:
  - added compatibility test for legacy payload where order state is stored as numeric enum value (`"2"`).
  - asserted mapping to `OrderStateType.Active` in `SetOrderFromString()`.
- Scope:
  - test-only hardening
  - no production runtime behavior changes.
...
- Host-context verification (outside sandbox):
  - `dotnet test project/OsEngine.Tests/OsEngine.Tests.csproj --no-restore --configuration Release --nologo` -> passed `432/432`

## 2026-02-27 - Step 2.2 (InvariantCulture) - Order numeric-cancel-state payload compatibility coverage block

- Expanded tests for `Order` persistence parsing paths:
  - `project/OsEngine.Tests/OrderPersistenceTests.cs`
- Changes:
  - added compatibility test for legacy payload where order state is stored as numeric enum value (`"7"`).
  - asserted mapping to `OrderStateType.Cancel` in `SetOrderFromString()`.
- Scope:
  - test-only hardening
  - no production runtime behavior changes.
...
## 2026-02-27 - Step 2.2 (InvariantCulture) - Order numeric-done-state payload compatibility coverage block

- Expanded tests for `Order` persistence parsing paths:
  - `project/OsEngine.Tests/OrderPersistenceTests.cs`
- Changes:
  - added compatibility test for legacy payload where order state is stored as numeric enum value (`"4"`).
  - asserted mapping to `OrderStateType.Done` in `SetOrderFromString()`.
- Scope:
  - test-only hardening
  - no production runtime behavior changes.
...
- Host-context verification (outside sandbox):
  - `dotnet test project/OsEngine.Tests/OsEngine.Tests.csproj --no-restore --configuration Release --nologo` -> passed `436/436`

## 2026-02-27 - Step 2.2 (InvariantCulture) - Order numeric-lost-after-active-state payload compatibility coverage block

- Expanded tests for `Order` persistence parsing paths:
  - `project/OsEngine.Tests/OrderPersistenceTests.cs`
- Changes:
  - added compatibility test for legacy payload where order state is stored as numeric enum value (`"8"`).
  - asserted mapping to `OrderStateType.LostAfterActive` in `SetOrderFromString()`.
- Scope:
  - test-only hardening
  - no production runtime behavior changes.
...
- Host-context verification (outside sandbox):
  - `dotnet test project/OsEngine.Tests/OsEngine.Tests.csproj --no-restore --configuration Release --nologo` -> passed `437/437`

## 2026-02-27 - Step 2.2 (InvariantCulture) - Order numeric-none-state payload compatibility coverage block

- Expanded tests for `Order` persistence parsing paths:
  - `project/OsEngine.Tests/OrderPersistenceTests.cs`
- Changes:
  - added compatibility test for legacy payload where order state is stored as numeric enum value (`"1"`).
  - asserted mapping to `OrderStateType.None` in `SetOrderFromString()`.
- Scope:
  - test-only hardening
  - no production runtime behavior changes.
...
- Host-context verification (outside sandbox):
  - `dotnet test project/OsEngine.Tests/OsEngine.Tests.csproj --no-restore --configuration Release --nologo` -> passed `438/438`

## 2026-02-28 - Step 2.2 (InvariantCulture) - Order numeric-order-type-time payload compatibility coverage block

- Expanded tests for `Order` persistence parsing paths:
  - `project/OsEngine.Tests/OrderPersistenceTests.cs`
- Changes:
  - added compatibility test for legacy payload where `OrderTypeTime` is stored as numeric enum value (`"2"`).
  - asserted mapping to `OrderTypeTime.Day` in `SetOrderFromString()`.
- Scope:
  - test-only hardening
  - no production runtime behavior changes.
...
## 2026-02-27 - Step 4.2 (nullable annotations) - TradeGridStopBy parser/log contract cleanup (`#642`)
...
- Changes:
  ...
  - parser robustness:
    - added index/empty checks before each field parse from `Split('@')` payload.
    - prevents `IndexOutOfRangeException` on truncated or malformed legacy strings.
  - log-event contract and dispatch:
    - `LogMessageEvent` -> nullable event (`Action<string, LogMessageType>?`)
    - dispatch switched to `LogMessageEvent?.Invoke(message, type)`
    - existing fallback to `ServerMaster.SendNewLogMessage(...)` for `Error` without subscribers preserved.
- Scope:
...
## 2026-02-27 - Step 4.2 (nullable annotations) - TradeGridStopAndProfit parser/log contract cleanup (`#643`)
...
- Changes:
  ...
  - parser robustness:
    ...
    - avoids index overflow on truncated/malformed settings strings.
  - log-event contract and dispatch:
    - `LogMessageEvent` -> nullable event (`Action<string, LogMessageType>?`)
...
## 2026-02-28 - Step 4.2 (nullable annotations) - TradeGrid lifecycle guard cleanup in internal helper paths (#652)

- Applied localized nullable-safe lifecycle hardening in:
  - project/OsEngine/OsTrader/Grids/TradeGrid.cs
- Changes:
  - added `GridCreator` null guards in internal lifecycle-sensitive methods:
  ...
  - loops now operate via guarded local `gridCreator` snapshots.
  - behavior for valid initialized runtime state preserved.
- Added/updated tests:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
    - `...PrivateLifecycleMethods_WithNullGridCreator_ShouldNotThrow`
    - reflection helper for private no-arg method invoke
- Scope:
  - nullable lifecycle guard cleanup only
  - no changes in trading decision logic.
...
## 2026-02-28 - Step 4.2 (nullable annotations) - TradeGrid save/load lifecycle guard cleanup (#655)

- Applied localized nullable-safe lifecycle hardening in:
  - project/OsEngine/OsTrader/Grids/TradeGrid.cs
- Changes:
  - `GetSaveString()` now uses guarded local snapshots and nullable-safe save fragments for optional subcomponents:
    - `NonTradePeriods`
    - `StopBy`
    - `GridCreator`
    - `StopAndProfit`
    ...
    - `ErrorsReaction`
    - `TrailingUp`
  - `LoadFromString(string? value)` now skips section-load calls when corresponding subcomponent was already cleared by lifecycle (`Delete()`), instead of dereferencing null.
  - behavior for initialized runtime state/payload format preserved.
- Added/updated tests:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
...
## 2026-02-28 - Step 4.2 (nullable annotations) - TradeGrid delete idempotency lifecycle cleanup (#661)

- Applied localized nullable-safe lifecycle hardening in:
  - project/OsEngine/OsTrader/Grids/TradeGrid.cs
- Changes:
  - `Delete()` moved to local snapshots for all lifecycle-owned subcomponents:
    - `Tab`
    - `NonTradePeriods`
    - `StopBy`
    - `StopAndProfit`
    ...
    - `ErrorsReaction`
    - `TrailingUp`
  - object fields are nulled before cleanup/unsubscription, making repeated `Delete()` calls idempotent and race-safer.
  - existing cleanup behavior preserved for initialized state.
- Added/updated tests:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
...
## 2026-02-28 - Step 4.2 (nullable annotations) - TradeGrid delete fail-safe cleanup without modal fallback (#662)

- Applied localized nullable-safe lifecycle hardening in:
  - project/OsEngine/OsTrader/Grids/TradeGrid.cs
- Changes:
  - `Delete()` cleanup blocks switched to fail-safe `try/catch` without error UI logging in defensive-path exceptions.
  - this prevents modal popups during cleanup of partially initialized/uninitialized subcomponents while preserving normal cleanup semantics.
  - retained idempotent/snapshot-based cleanup structure from prior increment.
- Added/updated tests:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
...
## 2026-02-28 - Step 4.2 (nullable annotations) - TradeGrid RemoveSelected null-lines lifecycle guard cleanup (#664)

- Applied localized nullable-safe lifecycle hardening in:
  - project/OsEngine/OsTrader/Grids/TradeGrid.cs
- Changes:
  - `RemoveSelected(List<int> numbers)` now guards `gridCreator.Lines` against null/empty before index-based access.
  - switched loop internals to local `lines` snapshot for safer access.
  - prevents null-reference in partial lifecycle/uninitialized `TradeGridCreator` states.
- Added/updated tests:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
...
## 2026-02-28 - Step 4.2 (nullable annotations) - TradeGrid entry-point/log payload hardening block (#668)

- Applied localized nullable-safe lifecycle hardening in:
  - project/OsEngine/OsTrader/Grids/TradeGrid.cs
- Changes:
  - added null payload guards in entry events:
    - `Tab_NewTickEvent(Trade trade)`
    - `Tab_PositionStopActivateEvent(Position obj)`
  - hardened `SendNewLogMessage(string message, LogMessageType type)`:
    - null-safe normalization for `message`;
    - uses local `tab` snapshot for bot/security context resolution.
  - behavior for valid runtime payloads preserved.
- Added/updated tests:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
...
## 2026-02-28 - Step 4.2 (nullable annotations) - TradeGrid process/journal lines-snapshot guard block (#669)

- Applied localized nullable-safe lifecycle hardening in:
  - project/OsEngine/OsTrader/Grids/TradeGrid.cs
- Changes:
  - `Process()` now snapshots `gridCreator.Lines` once and validates `lines == null/empty` before continuing.
  - `TryDeletePositionsFromJournal(Position position)` now returns early when `gridCreator.Lines` is null/empty.
  - reduces race window and avoids implicit null-loop semantics in partial lifecycle state.
- Added/updated tests:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
...
## 2026-02-28 - Step 4.2 (nullable annotations) - TradeGrid reconnect journal null-entry guard block (#672)

- Applied localized nullable-safe lifecycle hardening in:
  - project/OsEngine/OsTrader/Grids/TradeGrid.cs
- Changes:
  - hardened `TryFindPositionsInJournalAfterReconnect()` inner scan loop:
    - added null-entry guard for `positions[j]` before dereferencing `Number`.
    - kept existing semantics for successful match and cleanup fallback.
  - removes null-reference risk when journal position list contains sparse/null entries.
- Added/updated tests:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
...
## 2026-02-28 - Step 4.2 (nullable annotations) - TradeGridStopAndProfit process null-context guard block (#675)

- Applied localized nullable-safe lifecycle hardening in:
  - project/OsEngine/OsTrader/Grids/TradeGridStopAndProfit.cs
- Changes:
  - `Process(TradeGrid grid)` now uses local dependency snapshots and early return guards for `grid`, `grid.GridCreator`, `grid.Tab`, and `grid.Tab.Security`.
  - prevents null-reference in partial lifecycle/runtime states before profit/stop/trailing operations.
- Added/updated tests:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
...
## 2026-02-28 - Step 4.2 (nullable annotations) - TradeGridStopBy null-last-candle guard block (#679)

- Applied localized nullable-safe lifecycle hardening in:
  - project/OsEngine/OsTrader/Grids/TradeGridStopBy.cs
- Changes:
  - `GetRegime(...)` now guards null last candle entry before close-price access.
  - prevents null-reference when candle stream contains sparse/null tail element.
- Added/updated tests:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
...
## 2026-02-28 - Step 4.2 (nullable annotations) - TradeGridStopAndProfit null-last-candle trailing guard block (#680)

- Applied localized nullable-safe lifecycle hardening in:
  - project/OsEngine/OsTrader/Grids/TradeGridStopAndProfit.cs
- Changes:
  - `SetTrailStop(...)` now guards null last candle entry before close-price access.
...
## 2026-03-01 - Step 4.2 (nullable annotations) - TradeGridNonTradePeriods service contract coverage (#900-#901)

- Added targeted regression coverage in:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
- Changes:
  - locked the remaining service/save contracts for `TradeGridNonTradePeriods`.
  - covered stable reserved-tail save shape and idempotent `Delete()` cleanup.
- Added/updated tests:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
...
## 2026-03-01 - Step 4.2 (nullable annotations) - TradeGrid event dispatch positive coverage (#902-#903)

- Added targeted regression coverage in:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
- Changes:
  - locked the positive dispatch paths for `TradeGrid` public events and direct log forwarding.
  - covered `Save()`, `RePaintGrid()`, `FullRePaintGrid()`, and `SendNewLogMessage(...)` with active subscribers.
- Added/updated tests:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
...
## 2026-03-01 - Step 4.2 (nullable annotations) - TradeGrid helper safe defaults coverage (#904-#905)

- Added targeted regression coverage in:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
- Changes:
  - locked two remaining helper methods to their safe default empty-list contracts under null/sparse state.
  - covered `GetOrdersBadLinesMaxCount()` and `GetCloseOrdersGridHole()` when upstream line sources are unavailable.
- Added/updated tests:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
...
## 2026-03-01 - Step 4.2 (nullable annotations) - TradeGrid bad-price helper default coverage (#906)

- Added targeted regression coverage in:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
- Changes:
  - locked `GetOrdersBadPriceToGrid()` to its safe empty-list default when order-line sources are unavailable.
- Added/updated tests:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
...
## 2026-03-01 - Step 4.2 (nullable annotations) - TradeGrid open-hole direct guard coverage (#907-#908)

- Added targeted regression coverage in:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
- Changes:
  - locked direct early-return guards in `GetOpenOrdersGridHole()`.
  - covered the explicit `null Tab` and `null GridCreator` entry conditions.
- Added/updated tests:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
...
- Component: `TradeGrid`
- Focus: lock direct helper early-return contracts in private trading helpers
- Added targeted regression coverage in:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
- Changes:
  - locked the explicit `return 0` contract for `TryRemoveWrongOrders()` when `Tab` / `GridCreator` are unavailable.
  - locked the explicit `return 0` contract for `TryCancelOpeningOrders()` when `Tab` is unavailable.
  - locked the explicit `return 0` contract for `TryCancelClosingOrders()` when `Tab` is unavailable.
  - locked the explicit no-op contract for `TrySetClosingOrders(...)` when `Tab.Security` is unavailable.
- Added/updated tests:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
...
- Component: `TradeGrid`
- Focus: lock volume query property contracts
- Added targeted regression coverage in:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
- Changes:
  - locked `OpenVolumeByLines` so it sums `OpenVolume` across multiple open positions.
  - locked `AllVolumeInLines` so it sums `TradeGridLine.Volume` while ignoring sparse `null` entries.
- Added/updated tests:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
...
- Component: `TradeGrid`
- Focus: lock trivial core getter contracts
- Added targeted regression coverage in:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
- Changes:
  - locked `OpenPositionsCount` as a direct pass-through over `_openPositionsBySession`.
  - locked `FirstTradeTime` as a direct pass-through over `_firstTradeTime`.
- Added/updated tests:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
...
- Component: `TradeGridErrorsReaction`
- Focus: lock service contracts for save/delete helpers
- Added targeted regression coverage in:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
- Changes:
  - locked `GetSaveString()` reserved-tail shape and invariant-culture numeric formatting.
  - locked `Delete()` so it clears `_myGrid` and remains idempotent.
- Added/updated tests:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
...
- Component: `TradeGridStopAndProfit`
- Focus: lock save-string reserved-tail contract
- Added targeted regression coverage in:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
- Changes:
  - locked `GetSaveString()` reserved-tail shape and invariant-culture numeric formatting.
...
- Component: `TradeGridAutoStarter`
- Focus: lock save-string reserved-tail contract
- Added targeted regression coverage in:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
- Changes:
...
- Component: `TradeGridStopBy`, `TradeGridCreator`, `TradeGridLine`
- Focus: lock remaining serialization/save-string contracts in one batch
- Added targeted regression coverage in:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
- Changes:
  - locked `TradeGridStopBy.GetSaveString()` reserved-tail shape and invariant-culture numeric formatting.
  - locked `TradeGridCreator.GetSaveString()` reserved-tail shape, including the extra trailing separator after lines payload.
  - locked `TradeGridLine.GetSaveStr()` invariant-culture formatting and trailing field separator.
- Added/updated tests:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
...
- Component: `TradeGrid`
- Focus: lock top-level composite save/delete service contracts
- Added targeted regression coverage in:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
- Changes:
  - locked `GetSaveString()` as the exact composition of the prime section and all `%`-separated child component payloads.
  - locked `Delete()` so a fully initialized grid clears top-level references, cascades cleanup into child components, and remains idempotent.
- Added/updated tests:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
...
- Component: `TradeGridCreator`
- Focus: lock `LoadLines()` null/blank early-return contract
- Added targeted regression coverage in:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
- Changes:
  - locked `LoadLines(null)`, `LoadLines(string.Empty)`, and `LoadLines(whitespace)` so they keep existing lines unchanged.
- Added/updated tests:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
...
- Component: `TradeGridLine`
- Focus: lock remaining `SetFromStr()` boundary contracts
- Added targeted regression coverage in:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
- Changes:
  - locked reject-path for whitespace-only payload.
  - locked behavior when `PositionNum` token is malformed: parse still succeeds and existing `PositionNum` is preserved.
- Added/updated tests:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
...
- Component: `TradeGrid`
- Focus: lock event helper no-subscriber safe path
- Added targeted regression coverage in:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
- Changes:
  - locked `Save()`, `RePaintGrid()`, and `FullRePaintGrid()` so they remain no-op safe when no handlers are subscribed.
- Added/updated tests:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
...
- Component: `TradeGridCreator`
- Focus: lock positive runtime paths of `GetVolume()`
- Added targeted regression coverage in:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
- Changes:
  - locked positive-path contract for `TradeGridVolumeType.ContractCurrency`.
  - locked positive-path contract for `TradeGridVolumeType.DepositPercent` with `TradeAssetInPortfolio = "Prime"`.
- Added/updated tests:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
...
## 2026-03-02 - Incremental Update #963-#965

### Scope

- Added a compact non-UI service-contract regression block for log forwarding in remaining grid helper components.
...
- Updated tests in:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
- Changes:
  - locked `TradeGridStopBy.SendNewLogMessage(...)` forwarding behavior when a subscriber is attached.
  - locked `TradeGridStopAndProfit.SendNewLogMessage(...)` forwarding behavior when a subscriber is attached.
  - locked `TradeGridAutoStarter.SendNewLogMessage(...)` forwarding behavior when a subscriber is attached.
- Added/updated tests:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
...
## 2026-03-02 - Incremental Update #994-#996

### Scope

- Added first explicit round-trip serializer coverage for isolated child grid components.
...
- Added/updated tests:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
    - added `Stage2Step2_2_TradeGridStopBy_GetSaveString_LoadFromString_ShouldRoundTrip`.
    - added `Stage2Step2_2_TradeGridStopAndProfit_GetSaveString_LoadFromString_ShouldRoundTrip`.
    - added `Stage2Step2_2_TradeGridAutoStarter_GetSaveString_LoadFromString_ShouldRoundTrip`.
...
## 2026-03-02 - Incremental Update #1001

### Scope

- Added the first full composite persistence round-trip contract for top-level `TradeGrid`.
...
- Updated tests in:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
- Changes:
  - locked `TradeGrid.GetSaveString() -> LoadFromString()` round-trip behavior across:
    - prime section
    - `NonTradePeriods`
    - `StopBy`
    - `GridCreator`
...
- Added/updated tests:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
    - added `Stage2Step2_2_TradeGrid_GetSaveString_LoadFromString_ShouldRoundTrip`.
...
- Host-context verification (outside sandbox, per dotnet-build-policy):
  ...
  - dotnet restore project/OsEngine.Tests/OsEngine.Tests.csproj --nologo -> success
  - dotnet build project/OsEngine/OsEngine.csproj --no-restore --configuration Release --nologo -p:NoWarn=NU1900 -> success, 0 warnings, 0 errors
  - dotnet test project/OsEngine.Tests/OsEngine.Tests.csproj --no-restore --configuration Release --nologo -> passed 831/831

## 2026-03-02 - Incremental Update #1002

### Scope

- Added sparse top-level serialization coverage for `TradeGrid.GetSaveString()` with missing child sections.
...
- Updated tests in:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
- Changes:
  - locked `TradeGrid.GetSaveString()` shape when all persisted child references are `null`.
  - verified the prime section still serializes raw field defaults and all child sections stay empty but present.
- Added/updated tests:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
    - added `Stage2Step2_2_TradeGrid_GetSaveString_WithNullChildSections_ShouldKeepEmptySectionShape`.
...
- Host-context verification (outside sandbox, per dotnet-build-policy):
  ...
  - dotnet restore project/OsEngine.Tests/OsEngine.Tests.csproj --nologo -> success
  - dotnet build project/OsEngine/OsEngine.csproj --no-restore --configuration Release --nologo -p:NoWarn=NU1900 -> success, 0 warnings, 0 errors
  - dotnet test project/OsEngine.Tests/OsEngine.Tests.csproj --no-restore --configuration Release --nologo -> passed 832/832

## 2026-03-02 - Incremental Update #1003

### Scope

- Added the sparse-load counterpart to the sparse-save top-level `TradeGrid` coverage.
...
- Updated tests in:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
- Changes:
  - locked `TradeGrid.LoadFromString(...)` behavior on a payload with empty child sections.
  - verified prime fields are parsed while already-initialized child components keep their existing values.
- Added/updated tests:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
    - added `Stage2Step2_2_TradeGrid_LoadFromString_WithEmptyChildSections_ShouldKeepExistingChildValues`.
...
- Host-context verification (outside sandbox, per dotnet-build-policy):
  ...
  - dotnet restore project/OsEngine.Tests/OsEngine.Tests.csproj --nologo -> success
  - dotnet build project/OsEngine/OsEngine.csproj --no-restore --configuration Release --nologo -p:NoWarn=NU1900 -> success, 0 warnings, 0 errors
  - dotnet test project/OsEngine.Tests/OsEngine.Tests.csproj --no-restore --configuration Release --nologo -> passed 833/833

## 2026-03-02 - Incremental Update #1004

### Scope

- Added null-child safe-load coverage for top-level `TradeGrid.LoadFromString(...)`.
...
- Updated tests in:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
- Changes:
  - locked `TradeGrid.LoadFromString(...)` behavior when persisted child payload sections are present but all corresponding child references are `null`.
  - verified prime fields still parse and no exception is thrown.
- Added/updated tests:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
    - added `Stage2Step2_2_TradeGrid_LoadFromString_WithNullChildReferencesAndPresentPayloads_ShouldApplyPrimeAndNotThrow`.
...
- Host-context verification (outside sandbox, per dotnet-build-policy):
  ...
  - dotnet restore project/OsEngine.Tests/OsEngine.Tests.csproj --nologo -> success
  - dotnet build project/OsEngine/OsEngine.csproj --no-restore --configuration Release --nologo -p:NoWarn=NU1900 -> success, 0 warnings, 0 errors
  - dotnet test project/OsEngine.Tests/OsEngine.Tests.csproj --no-restore --configuration Release --nologo -> passed 834/834

## 2026-03-02 - Incremental Update #1005

### Scope

- Added prime-only partial-load coverage for top-level `TradeGrid.LoadFromString(...)`.
...
- Updated tests in:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
- Changes:
  - locked behavior when the payload contains only the prime section and no `%`-separated child sections.
  - verified prime fields parse while existing child components keep their previous values.
- Added/updated tests:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
    - added `Stage2Step2_2_TradeGrid_LoadFromString_WithPrimeOnlyPayload_ShouldApplyPrimeAndKeepExistingChildValues`.
...
- Host-context verification (outside sandbox, per dotnet-build-policy):
  ...
  - dotnet restore project/OsEngine.Tests/OsEngine.Tests.csproj --nologo -> success
  - dotnet build project/OsEngine/OsEngine.csproj --no-restore --configuration Release --nologo -p:NoWarn=NU1900 -> success, 0 warnings, 0 errors
  - dotnet test project/OsEngine.Tests/OsEngine.Tests.csproj --no-restore --configuration Release --nologo -> passed 835/835

## 2026-03-02 - Incremental Update #1006

### Scope

- Added selective child-section partial-load coverage for top-level `TradeGrid.LoadFromString(...)`.
...
- Updated tests in:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
- Changes:
  - locked behavior when the payload contains the prime section plus only the `StopBy` child section.
  - verified the targeted child updates while unrelated child components keep their existing values.
- Added/updated tests:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
    - added `Stage2Step2_2_TradeGrid_LoadFromString_WithOnlyStopBySection_ShouldApplyPrimeAndTargetChildOnly`.
...
- Host-context verification (outside sandbox, per dotnet-build-policy):
  ...
  - dotnet restore project/OsEngine.Tests/OsEngine.Tests.csproj --nologo -> success
  - dotnet build project/OsEngine/OsEngine.csproj --no-restore --configuration Release --nologo -p:NoWarn=NU1900 -> success, 0 warnings, 0 errors
  - dotnet test project/OsEngine.Tests/OsEngine.Tests.csproj --no-restore --configuration Release --nologo -> passed 836/836

## 2026-03-02 - Incremental Update #1007

### Scope

- Added the next selective child-section partial-load contract for top-level `TradeGrid.LoadFromString(...)`.
...
- Updated tests in:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
- Changes:
  - locked behavior when the payload contains the prime section plus only the `GridCreator` child section.
  - verified `GridCreator` updates while unrelated child components keep their previous values.
- Added/updated tests:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
    - added `Stage2Step2_2_TradeGrid_LoadFromString_WithOnlyGridCreatorSection_ShouldApplyPrimeAndTargetChildOnly`.
...
- Host-context verification (outside sandbox, per dotnet-build-policy):
  ...
  - dotnet restore project/OsEngine.Tests/OsEngine.Tests.csproj --nologo -> success
  - dotnet build project/OsEngine/OsEngine.csproj --no-restore --configuration Release --nologo -p:NoWarn=NU1900 -> success, 0 warnings, 0 errors
  - dotnet test project/OsEngine.Tests/OsEngine.Tests.csproj --no-restore --configuration Release --nologo -> passed 837/837

## 2026-03-02 - Incremental Update #1008
...
- Updated tests in:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
- Changes:
  - locked behavior when the payload contains the prime section plus only the `StopAndProfit` child section.
  - verified `StopAndProfit` updates while unrelated child components keep their previous values.
- Added/updated tests:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
    - added `Stage2Step2_2_TradeGrid_LoadFromString_WithOnlyStopAndProfitSection_ShouldApplyPrimeAndTargetChildOnly`.
...
- Host-context verification (outside sandbox, per dotnet-build-policy):
  ...
  - dotnet restore project/OsEngine.Tests/OsEngine.Tests.csproj --nologo -> success
  - dotnet build project/OsEngine/OsEngine.csproj --no-restore --configuration Release --nologo -p:NoWarn=NU1900 -> success, 0 warnings, 0 errors
  - dotnet test project/OsEngine.Tests/OsEngine.Tests.csproj --no-restore --configuration Release --nologo -> passed 838/838

## 2026-03-02 - Incremental Update #1009
...
- Updated tests in:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
- Changes:
  - locked behavior when the payload contains the prime section plus only the `AutoStarter` child section.
  - verified `AutoStarter` updates while unrelated child components keep their previous values.
- Added/updated tests:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
    - added `Stage2Step2_2_TradeGrid_LoadFromString_WithOnlyAutoStarterSection_ShouldApplyPrimeAndTargetChildOnly`.
...
- Host-context verification (outside sandbox, per dotnet-build-policy):
  ...
  - dotnet restore project/OsEngine.Tests/OsEngine.Tests.csproj --nologo -> success
  - dotnet build project/OsEngine/OsEngine.csproj --no-restore --configuration Release --nologo -p:NoWarn=NU1900 -> success, 0 warnings, 0 errors
  - dotnet test project/OsEngine.Tests/OsEngine.Tests.csproj --no-restore --configuration Release --nologo -> passed 839/839

## 2026-03-02 - Incremental Update #1010
...
- Updated tests in:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
- Changes:
  - locked behavior when the payload contains the prime section plus only the `ErrorsReaction` child section.
  - verified `ErrorsReaction` updates while unrelated child components keep their previous values.
- Added/updated tests:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
    - added `Stage2Step2_2_TradeGrid_LoadFromString_WithOnlyErrorsReactionSection_ShouldApplyPrimeAndTargetChildOnly`.
...
- Host-context verification (outside sandbox, per dotnet-build-policy):
  ...
  - dotnet restore project/OsEngine.Tests/OsEngine.Tests.csproj --nologo -> success
  - dotnet build project/OsEngine/OsEngine.csproj --no-restore --configuration Release --nologo -p:NoWarn=NU1900 -> success, 0 warnings, 0 errors
  - dotnet test project/OsEngine.Tests/OsEngine.Tests.csproj --no-restore --configuration Release --nologo -> passed 840/840

## 2026-03-02 - Incremental Update #1011
...
- Updated tests in:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
- Changes:
  - locked behavior when the payload contains the prime section plus only the `TrailingUp` child section.
  - verified `TrailingUp` updates while unrelated child components keep their previous values.
- Added/updated tests:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
    - added `Stage2Step2_2_TradeGrid_LoadFromString_WithOnlyTrailingUpSection_ShouldApplyPrimeAndTargetChildOnly`.
...
- Host-context verification (outside sandbox, per dotnet-build-policy):
  ...
  - dotnet restore project/OsEngine.Tests/OsEngine.Tests.csproj --nologo -> success
  - dotnet build project/OsEngine/OsEngine.csproj --no-restore --configuration Release --nologo -p:NoWarn=NU1900 -> success, 0 warnings, 0 errors
  - dotnet test project/OsEngine.Tests/OsEngine.Tests.csproj --no-restore --configuration Release --nologo -> passed 841/841

## 2026-03-02 - Incremental Update #1012
...
- Updated tests in:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
- Changes:
  - locked behavior when the payload contains the prime section plus only the `NonTradePeriods` child section.
  - verified `NonTradePeriods` updates while unrelated child components keep their previous values.
- Added/updated tests:
  - project/OsEngine.Tests/TradeGridPersistenceCoreTests.cs
    - added `Stage2Step2_2_TradeGrid_LoadFromString_WithOnlyNonTradePeriodsSection_ShouldApplyPrimeAndTargetChildOnly`.
...
## 2026-03-03 - Incremental Update #1023

### Scope

- Wave `P1` continuation: reduced transient allocations in TradeGrid wrong-order helper chain and tightened close-order fact-list capacity fallback in query hot path.
...
- Changes:
  - `TryRemoveWrongOrders()` now reuses local snapshots:
    - `GetLinesWithOpenOrdersFact()` is computed once and reused across bad-price, max-count, and grid-hole checks.
    - `GetLinesWithClosingOrdersFact()` is computed once for bad-price check.
  - introduced internal helper methods to consume precomputed snapshots without changing existing private method contracts used by tests:
    - `GetOrdersBadPriceToGridFromLines(...)`
    - `GetOrdersBadLinesMaxCountFromLines(...)`
    - `GetOpenOrdersGridHoleFromLines(...)`
  - `GetOrdersBadLinesMaxCountFromLines(...)` now pre-sizes cancellation list when overflow is known and exits early for empty/non-overflow cases.
  - `GetOpenOrdersGridHoleFromLines(...)` now allocates result list only when a cancellation candidate actually exists (single-item path).
  - `GetLinesWithClosingOrdersFact()` capacity heuristic now has fallback when `MaxCloseOrdersInMarket <= 0`:
    - bounded by `MaxOpenOrdersInMarket` (`min(linesAll.Count, max(MaxOpenOrdersInMarket * 2, 8))`) to avoid default full-grid over-allocation.
- Updated perf artifacts:
  - reports/stage2_perf_metrics.jsonl
  - reports/stage2_perf_summary.json
- Updated global coverage matrix:
...
### P0/P1 Metrics Delta (median, Repeat=5)
...
## 2026-03-03 - Incremental Update #1027

### Scope

- Wave `P2` continuation: remove source-id string conversion in optimizer cache key builders and add dedicated key-build KPI scenario.
...
- Updated tests/perf harness:
  ...
  - tools/perf-thresholds.json
- Changes:
  - added `int sourceId` key constructors for `IndicatorCacheKey` and `OptimizerMethodCacheKey` (existing `string sourceId` constructors retained for compatibility).
  - switched optimizer key-build callsites to `int sourceId`:
    - `Aindicator.BuildOptimizerIndicatorCacheKey(...)`
    - `BotPanel.BuildOptimizerMethodCacheKey(...)`
  - kept precomputed key hash behavior and extended hashing/equality paths for token-based source-id mode.
  - retained cached optimizer parameter-hash logic in `Aindicator`.
...
- Perf command:
  - pwsh -NoProfile -File tools/run-stage2-perf.ps1 -NoBuild -EnforceThresholds -Repeat 5 -> success
  - threshold check passed for all scenarios, including new key-build KPI.
- Host-context verification (outside sandbox, per dotnet-build-policy):
  - dotnet restore project/OsEngine/OsEngine.csproj --nologo -> success
  - dotnet restore project/OsEngine.Tests/OsEngine.Tests.csproj --nologo -> success
  - dotnet build project/OsEngine/OsEngine.csproj --no-restore --configuration Release --nologo -p:NoWarn=NU1900 -> success, 0 warnings, 0 errors
...
- `indicator_cache_hit_path`:
  - current `#1027`: `1900.40 ns/op`, `448.02 bytes/op`
- `optimizer_method_cache_hit_path`:
  - current `#1027`: `148.82 ns/op`, `0.01 bytes/op`
- `optimizer_cache_key_build_path` (new):
